/*
 *
 *  *  com.sonicle.commons.qbuilders.visitors.ContextualNodeVisitor
 *  *  *
...
public abstract class ContextualNodeVisitor<T, S> {
	...
	/**
	 * Build a comparison node value into a visited value so that it can be
	 * composed into the larger query being built.
	 *
	 * @param node The node with a condition argument to build into a visited
	 * value.
	 *
	...
	protected Object[] multiple(Collection<?> values, int howMany) {
		if (!values.isEmpty()) {
